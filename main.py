...
def submit(var,reading_style,username,password,sexe,age):
    ...
    if index ==0:
        ...
        Submit=Button(top,text="Confirmer",command=lambda: [clear(),modify_reader(index,username,age,sexe,img_nbr,liste_like,reading_style),fenetremain()])
        Submit.grid(row=10,column=0)
...
def notation(username,age,sexe,img_nbr,liste_like,reading_style,rating):
    ...
    add_readers(username,age,sexe,img_nbr,liste_like,reading_style,liste_note)
...
def sceneprofile():
    ...
    for i in range(len(liste_livre)):                                                                       #For all of the existing books
        if liste_livre[i]!="":                                                                              #Making sure there is acutally a name 
            var.append(IntVar())
            check.append(Checkbutton(fenetre,text=liste_livre[p],variable=var[p],offvalue=0,onvalue=1))     #Generating checkbutton with intvar in a list
            check[p].grid(row=(k),column=j)                                                                 #Placing all of the check button
            j+=1
            p+=1
...
def popup(y):
    ...
    Button(top,text="Confirmer",command=lambda:[modify_book(liste_livre[y],nouveau_nom.get()),top.destroy(),scenebook()]).pack(side=LEFT)
...
def scenebook():
    ...
    entrer = Button(fenetre,text="Entrer",command=lambda:[add_book(livre.get()),livre.set(""),scenebook()])
...
def add_readers(name, age, genre, img_nbr, liste_like, reading_style,liste_note):
...
def modify_book(nom_livre,nom_modify):
